...
const create = async (req, res, next) => {
    const dataToCreate = new Category(req.body);
...
const update = async (req, res, next) => {
    ...
    categoryUpd=await Category.findOneAndUpdate({_id:req.body._id}, {
...
const remove = async (req, res, next) => {
    Category.deleteOne({_id: req.params._id}, function (err, doc) {
        if (err) {
            res.json(myLib.sendResponse(0))
            return
        }
